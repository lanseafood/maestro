...
module.exports = class TemplateTransform {
	...
	replaceTemplate(subject, templateName, replacerFn, transformArgs = true) {
		...
		if (!(/[a-zA-Z_]+/g).test(templateName)) {
		...
		fullTemplateCall();
		// const args = match[2];
		// like "| some actionYYY" FIXME: this only looks for one arg now

		const prefix = subject.substring(0, match.index); // trim everything up to the string

		// const preTransformed = somehowGetStringForTemplateCall();
		// I made this up for now

		// const suffix = match.index + preTransformed.length;
		// guess
